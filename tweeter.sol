...
contract TweetContract{
    ...
    function _tweet(address _from,string memory
    _content) internal{//tweet access check-owner,authority
    ...
    function _sendMessages(address _from,address _to,string memory _content) internal{ //tweet access check - owner,authority
    require(_from==msg.sender || operators[_from][msg.sender],"You don't have access");
    ...
    function tweet(address _from,string memory 
    _content) public{//owner->address access
    _tweet(_from,_content);
    }

    function _sendMessage(string memory
    _content,address _to) public {//owner 
    ...
    function getLatestTweets(uint count)public view returns(Tweet[] memory){
        ...
        uint j;
        for(uint i=nextId-count;i<nextId;i++){//count=5
        // nextId=7;7-5=2,3,4,5,6
